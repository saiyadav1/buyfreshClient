...
const SellerOrders = () => {
    ...
    async function getmyorders() {
        try {
            const response = await axios.get(`${api}/sellergetMyOrders`, {
                params: {
                    type: 'SELLER',
                    seller_id: user_state.user_id,
                }
            });
    ...
    function viewOrder(item){
        navigate(`/seller/order/${item.orderId}`);
    }
    return (
        <div className='orderDetails-container'>
            ...
            <div className='orderDetails-body-container'>
                {orderData&& orderData.map(item => {
                    return (
                        <div className='orderDetails-body' onClick={()=>viewOrder(item)}>
